...
ALTER TABLE "rbac"."roles" ALTER COLUMN "id" ADD GENERATED BY DEFAULT AS IDENTITY (
    SEQUENCE NAME "rbac"."roles_id_seq"
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
ALTER TABLE "rbac"."user_role" OWNER TO "postgres";
...
ALTER TABLE ONLY "rbac"."user_role"
...
ALTER TABLE ONLY "rbac"."user_role"
    ADD CONSTRAINT "rbac_user_role_id_fkey" FOREIGN KEY ("user_id") REFERENCES "auth"."users"("id");

ALTER TABLE ONLY "rbac"."user_role"
    ADD CONSTRAINT "rbac_user_role_role_fkey" FOREIGN KEY ("role") REFERENCES "rbac"."roles"("id");

CREATE POLICY "Enable read access for all users" ON "public"."attendance_records" FOR SELECT TO "authenticated" USING (true);

CREATE POLICY "Enable read access for all users" ON "public"."classes" FOR SELECT TO "authenticated" USING (true);

CREATE POLICY "Enable read access for all users" ON "public"."students" FOR SELECT TO "authenticated" USING (true);

CREATE POLICY "Enable read access for all users" ON "public"."teachers" FOR SELECT TO "authenticated" USING (true);

CREATE POLICY "Allow auth admin to read user roles" ON "rbac"."user_role" FOR SELECT TO "supabase_auth_admin" USING (true);
...
GRANT ALL ON TABLE "rbac"."user_role" TO "supabase_auth_admin";
