...
export const useAttendanceStats = () => {
  ...
  const attendanceDataWithPercentage: StudentAttendanceDataWithPercentage[] = useMemo(() => {
    return allStudentAttendanceData.map((data) => {
      const { totalAttendance, presentAttendance } = attendanceDataByTime[data.student.scholar_id] || { totalAttendance: 0, presentAttendance: 0 };
    ...
    const sortFunction = (a: StudentAttendanceDataWithPercentage | ClassData, b: StudentAttendanceDataWithPercentage | ClassData) => {
      if (selectedButton === 'right') {
        switch (sortOption) {
          ...
          case 'Name: Z to A':
            return (b as StudentAttendanceDataWithPercentage).student.student_name.localeCompare((a as StudentAttendanceDataWithPercentage).student.student_name);
          ...
          case 'Class: Low to High':
            return compareClasses((a as StudentAttendanceDataWithPercentage).class_name, (b as StudentAttendanceDataWithPercentage).class_name);
          case 'Class: High to Low':
            return compareClasses((b as StudentAttendanceDataWithPercentage).class_name, (a as StudentAttendanceDataWithPercentage).class_name);
          default:
            return 0;
        }
      } else {
  ...
  const filteredAttendanceData = useMemo(() => {
    ...
    const filterFunction = (data: StudentAttendanceDataWithPercentage | ClassData) => {
      if (selectedButton === 'right') {
        ...
        const studentName = (data as StudentAttendanceDataWithPercentage).student.student_name.toLowerCase();
